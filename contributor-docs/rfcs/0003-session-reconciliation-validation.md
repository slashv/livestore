...
Status: validation of the local serialized-sync proposal, not an accepted replacement for the repository's intent.
See [the architecture RFC](./0003-serialized-sync-processors.md#safe-session-reconciliation-steps).
...
- [ ] Decide whether the measured large-pending cost warrants a follow-up optimization before adoption.
